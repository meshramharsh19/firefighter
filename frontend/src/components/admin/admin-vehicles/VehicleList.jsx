...
export default function VehicleList({ vehicles: initialVehicles, onUpdated }) {
  ...
  const handleEditSave = async (updated) => {
    try{
      ...
      if(data.success){
        ...
        // 🟢 notify parent → stats + card refresh without reload
        onUpdated && onUpdated()
  ...
  if(!vehicles.length){
    return(
      <Card><CardContent className="p-12 text-center">No Data</CardContent></Card>
    )
  }
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="flex items-center gap-2">
                <div className="rounded-lg bg-primary/10 p-2">
                  <SafeIcon name={getVehicleTypeIcon(vehicle.type)} className="text-primary"/>
                </div>
                <div>
              ...
              <div>
                ...
                <p className="flex items-center gap-1"><SafeIcon name="MapPin" size={14}/> {vehicle.location}</p>

                <p className="text-xs text-muted-foreground mt-2">Ward</p>
                <Badge variant="outline" className="text-xs">Ward {vehicle.ward}</Badge>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2 mt-2">
                  <Button size="sm" variant="outline" 
                    onClick={()=>window.location.href=`./asset-details.html?id=${vehicle.id}`}>
                    View
                  </Button>

                  <Button size="sm" variant="ghost" onClick={()=>setEditVehicle(vehicle)}>
                    Edit
                  </Button>
                </div>
              </div>
