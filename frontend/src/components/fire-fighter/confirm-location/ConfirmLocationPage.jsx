...
export default function ConfirmLocationPage() {
  ...
  useEffect(() => {
    fetch(`${API}/incidents/get_incidents.php`)
      ...
      .then(data => {
        ...
        if (!inc) return alert("Incident not found");
  ...
  const stations = [
    { id:"FS-112", name:"Central Station", distance:1.5, vehicles:6 },
      ...
      <Box sx={{ minHeight:"100vh", p:3 }}>
        <Stack spacing={4} maxWidth="1200px" mx="auto">
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            <Box sx={{ width:44, height:44, borderRadius:2, bgcolor:"#1E1E1F",
          ...
          {/* Incident Card */}
          <Card>
            <CardHeader title={<Typography variant="h6">{incident.name}</Typography>} />
            <CardContent>
              <Stack direction="row" spacing={4} flexWrap="wrap">
                <InfoField label="Incident ID" value={incident.id} mono />
                <InfoField label="Type" value={incident.type} mono />
                <InfoField label="Status" value={<Chip size="small" label={incident.status} color="error" />} />
          ...
          <Stack direction={{xs:"column", lg:"row"}} spacing={3}>
            ...
            <Box flex={2}>
              <MapWithDraggableMarker
                ...
                onMarkerMove={(lat,lng)=>{ setCurrentLat(lat); setCurrentLng(lng); setHasMarkerMoved(true); }}
              />
            </Box>
                ...
                <Card sx={{ borderColor:"primary.main", bgcolor:"#211112" }}>
                  <CardContent>
                    <Stack direction="row" spacing={2}>
                      ...
                      <Box>
                        <Typography fontWeight="bold">Location Adjusted</Typography>
                        <Typography variant="caption">Routing & response units recalculated</Typography>
                      </Box>
                    </Stack>
                  </CardContent>
                </Card>
...
function InfoField({label, value, mono}) {
  return (
    <Box>
      ...
      <Typography variant="body2" sx={{ fontFamily: mono?"monospace":"inherit", fontWeight:500 }}>
        {value}
      </Typography>
    </Box>
  );
